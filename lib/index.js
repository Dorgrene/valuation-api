...
server.route({
  ...
  handler: function (request, reply) {
    ...
    YahooFinance.quote({
      ...
      const lastYearEarning = lastThreeYearEarnings[lastThreeYearEarnings.length-1].earnings
      const totalDebt = snapshot.financialData.totalDebt
...
server.route({
  ...
  handler: function (request, reply) {
    ...
    YahooFinance.historical({
      ...
      reply(_.map(result, symbolData => Object.assign({allocation: +(coefficient/symbolData.histVolatility).toFixed(3)}, symbolData)))
    });
  }
})
